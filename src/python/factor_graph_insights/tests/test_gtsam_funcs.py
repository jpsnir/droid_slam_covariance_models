...
def test_Cal3_S2_camera_model_constructor():
    ...
    assert cam.fy() == 1
    assert cam.skew() == 0
    print(f'type of property value principal point:\
            {type(cam.principalPoint())}, shape = {cam.principalPoint().shape}')
    np.testing.assert_equal(cam.principalPoint(), np.array([0., 0.]))
...
def test_Cal3_S2_camera_calibrate_method():
    ...
    for sample in x:
        ...
        print(f'Output of pt({pt}) with  calibrate function:\
                {cam.calibrate(pt)}, shape = {cam.calibrate(pt).shape}')

    p_xy = gtsam.Point2(30, 40)
    normalized_coords = np.array([(p_xy[0] - cam.px()) / cam.fx(),
                                  (p_xy[1] - cam.py()) / cam.fy()])
...
def test_Cal3_S2_camera_uncalibrate_method():
    ...
    normalized_coords = np.array([[1, 0],
                                  [0, 1],
                                  [1, 1],
                                  [-1, 0],
                                  [0, -1],
...
def test_pinhole_perspective_camera_functions():
    ...
    pt_3d_list = [gtsam.Point3(x=0, y=0, z=10),
    ...
    chiral_pts_3d = [gtsam.Point3(x=0, y=5, z=0),
