...
class Visualizer:
    ...
    def __init__(self, metadata: Dict = None):
        ...
        if metadata is not None:
            ...
            self.file_num = self.metadata['file_id']     
    
    def plot_trends_determinants(self, Determinant, adjacency_matrix_data: Dict = None):
        """
        """
        ...
        ax[0].set_xticks(self.image_ids[[node_ids]])
        ax[0].xaxis.set_ticks(self.image_ids[[node_ids]])
        ...
        ax[0].set_title(f"{self.metadata['dataset_name']} - {len(node_ids)} keyframes\
            - {int(self.image_ids[-1].item()/20)}s", fontsize=18)
        ...
        if self.save_location is not None:
            fig.savefig(self.save_location.joinpath(f"{self.metadata['dataset_name']}.pdf"))
    ...
    def plot_trends_singular_values(self, singular_values:np.array):
        ...
        fig1.suptitle(f"Trends of singular Values of covariance matrix - angles\
            - {self.file_num}", fontsize=18)
        ...
        if self.save_location is not None:      
            fig1.savefig(self.save_location.joinpath(f"angle_{self.file_num}.png"))
            fig2.savefig(self.save_location.joinpath(f"position_{self.file_num}.png"))
...
class CovarianceResultsAnimator(AnimatorBase):
